...
function addGamesToPage(games) {
    ...
    for (let game of games) {
        ...
        newGame.innerHTML = `<img class='game-img' src=${game.img} />
                             <p><b>${game.name}</b></p>
...
let numberOfUnfunded = GAMES_JSON.reduce((acc, game) => {
    return game.pledged < game.goal ? acc + 1 : acc;
}, 0);

// create a string that explains the number of unfunded games using the ternary operator
let displayStr =
    `A total of $${totalAmountRaised} has been raised for ${GAMES_JSON.length} ${GAMES_JSON.length > 1 ? "games" : "game"}.
Currently, ${numberOfUnfunded} ${numberOfUnfunded > 1 ? "games remain" : "game remains"} unfunded.
...
const searchResult = document.getElementById("search_input");
const searchBtn = document.getElementById("search_btn");
searchBtn.addEventListener('click', () => {

    let regexp = new RegExp(searchResult.value.toLowerCase(), "g");
    console.log(regexp);
    let SearchedGames = GAMES_JSON.filter((game) => {
        return game.name.toLowerCase().match(regexp) ? true : false
    });
    deleteChildElements(gamesContainer);
